...
def write_dhcp_snooping_to_csv(config_list,output):
        regex=(r'((?:\S+:){5}\S+)\s+(\d+\.\d+\.\d+\.\d+)\s+\d+\s+\S+\s+(\d+)\s+(\S+)')
        header=['switch','mac','ip','vlan','interface']
        regex_name_dev=(r'(\S+\d)\_')
        result_list=[header]
        
        for member_list in config_list:
            with open(member_list) as f:
                net_dev = []
                for i in f:
                    match=re.search(regex,i)
                    match_name_dev=re.search(regex_name_dev,member_list)
                    if match_name_dev and match:
                       result_list.append(list(match_name_dev.groups(0) + match.groups(0)))
